...
class TestAudTxtSchema:
    def test_audtxt_schema(self, an_audtxt_path):
    ...
    def test_audtxt_schema_bad_df(self, an_audtxt_path):
        audtxt_df = pd.read_csv(an_audtxt_path, sep="\t", header=None)
...
def test_to_seq_round_times_true(test_data_root, an_audtxt_path, decimals):
...
def test_to_seq_round_times_false(test_data_root, an_audtxt_path):
    audtxt = crowsetta.formats.seq.AudTxt.from_file(annot_path=an_audtxt_path)
    seq = audtxt.to_seq(round_times=False)

    assert np.all(np.allclose(seq.onsets_s, audtxt.start_times))
    assert np.all(np.allclose(seq.offsets_s, audtxt.end_times))
...
def test_to_annot_round_times_true(test_data_root, an_audtxt_path, decimals):
    audtxt = crowsetta.formats.seq.AudTxt.from_file(annot_path=an_audtxt_path)
...
def test_to_annot_round_times_false(test_data_root, an_audtxt_path):
    audtxt = crowsetta.formats.seq.AudTxt.from_file(annot_path=an_audtxt_path)
    annot = audtxt.to_annot(round_times=False)

    assert np.all(np.allclose(annot.seq.onsets_s, audtxt.start_times))
    assert np.all(np.allclose(annot.seq.offsets_s, audtxt.end_times))


def test_to_file(an_audtxt_path, tmp_path):
    audtxt = crowsetta.formats.seq.AudTxt.from_file(annot_path=an_audtxt_path)
