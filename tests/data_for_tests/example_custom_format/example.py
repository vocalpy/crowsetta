...
class Custom:
    ...
    def to_seq(self):
        ...
        for filename, annotation in zip(self.audio_paths, self.annotations):
            ...
            if type(labels) == int:
                ...
                raise ValueError(
                    "Unable to load labels from {}, because "
                    "the segType parsed as type {} which is "
                    "not recognized.".format(filename, type(labels))
                )
    ...
    def to_annot(self):
        ...
        for filename, seq in zip(self.audio_paths, seqs):
            annot_list.append(Annotation(annot_path=self.annot_path, notated_path=filename, seq=seq))
