...
def test_as_dict_onset_offset_in_Hertz():
    ...
    assert np.all(seq_dict['onsets_Hz'] == onsets_Hz)
    assert np.all(seq_dict['offsets_Hz'] == offsets_Hz)
...
def test_as_dict_onset_offset_in_seconds():
    ...
    assert seq_dict['onsets_Hz'] is None
    assert seq_dict['offsets_Hz'] is None
